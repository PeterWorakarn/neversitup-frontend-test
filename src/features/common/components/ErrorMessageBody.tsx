import React from 'react'

type TErrorMessageProps = {
  message: string
}

const ErrorMessageBody: React.FC<TErrorMessageProps> = (props) => {
  return (
    <div className='flex items-center justify-start gap-1 mt-1.5'>
      <svg className="h-4 w-4 text-red-600" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" aria-hidden="true">
        <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z"></path>
      </svg>
      <p className='text-red-500 text-xs font-medium '>
        {props.message}
      </p>
    </div>
  )
}

export default ErrorMessageBody